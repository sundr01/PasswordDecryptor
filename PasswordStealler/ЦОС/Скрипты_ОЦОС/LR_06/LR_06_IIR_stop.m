...
clc
clear
disp('% кп ╧06. яхмрег пефейрнпмнцн аху-тхкэрпю лернднл ахкхмеимнцн Z-опенапюгнбюмхъ')
disp('%')
disp('% бБЕДХРЕ рпеанбюмхъ Й юву пт')
DATA=0;
while DATA==0
Nb = input('Nv = ');         % мнлеп бюпхюмрю
Fs = input('Fs (цЖ) = ');         % вюярнрю дхяйперхгюжхх Б цЖ
ft1 = input('fpass1 (цЖ) = ');       % цпюмхвмюъ вюярнрю оо1 Б цЖ
fk1 = input('fstop1 (цЖ) = ');       % цпюмхвмюъ вюярнрю ог1 Б цЖ
fk2 = input('fstop2 (цЖ) = ');       % цпюмхвмюъ вюярнрю ог2 Б цЖ
ft2 = input('fpass2 (цЖ) = ');       % цпюмхвмюъ вюярнрю оо2 Б цЖ
...
disp('%')
disp('% дКЪ ЯХМРЕГЮ пефейрнпмшу аху-ТХКЭРПНБ МЮФЛХРЕ <ENTER>')
pause
...
pause
disp('%')
disp('% О.2. юмюкхг уюпюйрепхярхй пефейрнпмшу аху-тхкэрпнб')
disp('%')
...
n = 0:(M-1);
f = 0:((Fs/2)/1000):Fs/2; 
figure('Name','пт аху-ТХКЭРП аЮРРЕПБНПРЮ','NumberTitle', 'off')
h1 = impz(b1,a1,M);         % ху пт аху-тхкэрпю аюррепбнпрю    
H1 = freqz(b1,a1,f,Fs);     % ву пт аху-тхкэрпю аюррепбнпрю
MAG = abs(H1);              % юву пт аху-тхкэрпю аюррепбнпрю
PHASE = phase(H1);          % тву пт аху-тхкэрпю аюррепбнпрю
...
figure('Name','пт аху-ТХКЭРП вЕАШЬЕБЮ I ПНДЮ','NumberTitle', 'off')
h2 = impz(b2,a2,M);         % ху пт аху-тхкэрпю веашьебю I пндю
H2 = freqz(b2,a2,f,Fs);     % ву пт аху-тхкэрпю веашьебю I пндю
MAG = abs(H2);              % юву пт аху-тхкэрпю веашьебю I пндю
PHASE = phase(H2);          % тву пт аху-тхкэрпю веашьебю I пндю
subplot(2,2,1), plot(f,MAG), xlabel('f (цЖ)')
title('юлокхрсдю'), grid, ylim([0 1.2])
...
xlabel('n'), title('хлоскэямюъ уюпюйрепхярхйю'), grid
figure('Name','пт аху-ТХКЭРП вЕАШЬЕБЮ II ПНДЮ','NumberTitle', 'off')
h3 = impz(b3,a3,M);         % ху пт аху-тхкэрпю веашьебю II пндю
H3 = freqz(b3,a3,f,Fs);     % ву пт аху-тхкэрпю веашьебю II пндю
MAG = abs(H3);              % юву пт аху-тхкэрпю веашьебю II пндю
PHASE = phase(H3);          % тву пт аху-тхкэрпю веашьебю II пндю
subplot(2,2,1), plot(f,MAG), xlabel('f (цЖ)')
title('юлокхрсдю'), grid, ylim([0 1.2])
...
xlabel('n'), title('хлоскэямюъ уюпюйрепхярхйю'), grid
figure('Name','пт аху-ТХКЭРП гНКНРНПЕБЮ-йЮСЩПЮ','NumberTitle', 'off')
h4 = impz(b4,a4,M);         % ху пт аху-тхкэрпю гнкнрюпебю-йюсщпю
H4 = freqz(b4,a4,f,Fs);     % ву пт аху-тхкэрпю гнкнрюпебю-йюсщпю
MAG = abs(H4);              % юву пт аху-тхкэрпю гнкнрюпебю-йюсщпю
PHASE = phase(H4);          % тву пт аху-тхкэрпю гнкнрюпебю-йюсщпю
subplot(2,2,1), plot(f,MAG), xlabel('f (цЖ)')
title('юлокхрсдю'), grid, ylim([0 1.2])
...
disp('%')
disp('% дКЪ БШБНДЮ ЦПЮМХВМШУ ВЮЯРНР юто пт МЮФЛХРЕ <ENTER>')
pause
disp('%')
...
figure('Name','юву пефейрнпмнцн тхкэрпю юто','NumberTitle', 'off')
...
disp('%')
disp('% яхмрег пефейрнпмнцн аху-тхкэрпю гюбепьем')
